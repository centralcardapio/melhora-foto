...
serve(async (req) => {
  ...
  try {
    const { email, name, dashboardLink } = await req.json()

    if (!email || !name || !dashboardLink) {
      return new Response(
        JSON.stringify({ error: 'Email, nome e link do dashboard são obrigatórios' }),
        {
          status: 400,
    ...
    // Embedded HTML template for welcome email
    let template = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Central do Cardápio - Bem-vindo!</title>
    <style>
...
<body>
    <div class="email-container">
        ...
        <div class="content">
            ...
            <h1 class="greeting">Bem-vindo, ${name}!</h1>
            <p class="message">
                Que alegria ter você conosco! Sua conta foi criada com sucesso e você já pode começar a transformar suas fotos de pratos em imagens profissionais.
            </p>
            ...
            <p class="message" style="margin-top: 30px;">
                Com nossa plataforma, você pode transformar qualquer foto de comida em uma imagem digna de restaurante gourmet. 
                Experimente diferentes estilos e veja a magia acontecer!
            </p>
        </div>
        <div class="footer">
    ...
    template = template.replace('{{DASHBOARD_LINK}}', dashboardLink)

    const result = await sendEmailViaSMTP(
      email,
      '🎉 Bem-vindo à Central do Cardápio!',
      template
    )
